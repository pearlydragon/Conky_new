...
noweather = 1;
	...
	if noweather == 1 then
		...
		if (tonumber(internet) == 1) then
			local file = io.popen("curl -m 120 'http://api.openweathermap.org/data/2.5/weather?q="..city_id.."&appid="..apikkey.."&units=metric'");
			weather = file:read("*a");
			io.close(file);
		end
		...
		if (weather == "") then
			...
			condition,code,temp,country,city = string.match(weather, "\"description\":\"(%a+.%a+)\".-\"icon\":\"(%w+)\".-\"temp\":([+-]?%d+)\,.-\"country\":\"(%a+)\".-\"name\":\"(%a-)\"");

			local file = io.popen("curl -m 120 'http://api.openweathermap.org/data/2.5/forecast/daily?q="..city_id.."&appid="..apikkey.."&units=metric&cnt=3'");
			weather = file:read("*a");
			io.close(file);
			local i = 0;

			for A,B,C,D in string.gmatch(weather, "\"dt\":(%d+)\,.-\"day\":(.-)\,.-\"night\":(.-)\,.-\"description\":\"(%a.-%a)\"") do
				i = i + 1;
				days[i] = A;
				highs[i] = B;
				lows[i] = C;
	...
	if noweather == 1 then
		...
		--if tonumber(code) < 10 then
			--code = "0"..code;
			--Из-за того, что иконка у openweathermap не строка из цифр такое больше не прокатит.
		--end
		...
		for i = 1,3 do
			text = os.date("%a", days[i]).." : "..highs[i].."°C / "..lows[i].."°C  "..texts[i];
