...
function conky_main() 
	...
	if tonumber(updates) > 1 then
		...
		for i = 1,10 do
			...
			cairo_move_to(cr,220,790+i*13);
			cairo_show_text(cr,name);
			cairo_move_to(cr,295,790+i*13);
		...
		for i = 1,10 do
			...
			cairo_move_to(cr,1090,-510+i*13);
			cairo_show_text(cr,name);
			cairo_move_to(cr,1165,-510+i*13);
			cairo_show_text(cr,value);
		end
		cairo_rotate(cr,-45*(math.pi/180));
	end
	...
	cairo_select_font_face(cr,"Arial",0,0);
	cairo_set_source_rgba(cr,1,1,1,1);
	local network = conky_parse("${addr enp2s0}");
	text = network;
	cairo_text_extents(cr,text,extents);
	...
	cairo_select_font_face(cr,"Arial",0,0);
	cairo_set_source_rgba(cr,1,1,1,1);
	local download = conky_parse("${downspeed enp2s0}");
	...
	cairo_select_font_face(cr,"Arial",0,0);
	cairo_set_source_rgba(cr,1,1,1,1);
	local upload = conky_parse("${upspeed enp2s0}");
